...
const FixtureResults = () => {
        ...
        if (uniqueTournament.name) {
            try {
                ...
                if (hasIndian(item)) {
                    ...
                    // </div>)
                    objDom = (<div className="flex flex-row w-full h-full text-sm sm:text-xs xs:text-xs space-x-2 sm:space-x-4">
    ...
    function getTournamentType(tournament) {
        ...
        let points = rankingsData[tournament][0]?.tournament?.uniqueTournament?.tennisPoints
        if (seasonName.includes("US Open")) {
            return seasonName
        }
        if (category === "Challenger") {
            if (name.toLowerCase().includes("qual")) {
                return seasonName + "- Qualifying"
            }
            else {
                return seasonName
            }
    ...
    function getScoreHeader(tournament) {
        let seasonName = rankingsData[tournament][0]?.season?.name
        let name = rankingsData[tournament][0]?.tournament?.name
        let type = seasonName.split(",")[0]
        let parsedName = getTournamentType(tournament)
        if (seasonName) {
            if (seasonName.includes("Men")) {
                return (<div className="flex flex-row bg-blue-300 items-center p-1 text-sm sm:text-base md:text-lg lg:text-lg xl:text-lg">
