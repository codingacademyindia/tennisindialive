...
def persist_live_rankings(conn, source_slug, rows):
    if conn is None:
        return
...
def fetch_ranking(url):
    ...
    if "atp" in file_name:
        file_name = f"ranking/live/atp/{file_name}.json"
    else:
        file_name = f"ranking/live/wta/{file_name}.json"
    driver.get(url)
    ...
    persist_live_rankings(db_conn, source_slug, data)
...
for url in urls:
    ...
    driver.close()
    driver.quit()
file_timestamp="ranking/live/live_ranking_timestamp.json"
