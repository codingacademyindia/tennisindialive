...
import { buildGroupedLiveMatchesTweet, formatLiveScoreTweet, getAllLiveMatchesFromFiltered } from './TweetFormatter';
...
const FixtureResultsAdmin = () => {
    ...
    const handleAllLive = () => {
        ...
        const filteredTournaments = Object.keys(rankingsDataCopy).filter(t =>
            rankingsDataCopy[t].some(hasCountry)
        );

        if (filteredTournaments.length === 0) {
            toast.info("No live matches found for the selected filters.");
            return;
        }
    ...
    const sendTweet = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setTweetStatus("fail:" + errMsg)
                console.log("Tweet Failed:", errMsg);
            }
        } catch (err) {
            // NETWORK ERROR
...
shadow-[0_0_6px_rgba(255,255,255,0.1)]">
  ...
  ">
             ...
             text-amber-300 text-[10px] sm:text-xs
             hover:bg-amber-500/20 hover:border-amber-300/50
             hover:shadow-[0_0_10px_rgba(255,190,100,0.4)]
             transition-all duration-200
             backdrop-blur-sm"
                    >
                        Tweet
    ...
    return (
        <div className="bg-gray-900 min-h-screen text-gray-200">
            ...
            <SEO
                ...
                description={`Admin dashboard for real-time tennis scores and Twitter posting for ${selectedCountry}. Follow ATP, WTA, and local tournaments.`}
                keywords={`tennis admin, tweet composer, ${countryFullName} tennis, live scores`}
                url={`https://tennisindialive.com/tennisadmin`}
            />
...
export default FixtureResultsAdmin;
